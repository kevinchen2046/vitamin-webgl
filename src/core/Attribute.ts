...
import { BufferType, ShaderBuffer } from "./ShaderBuffer";
...
export class Attribute extends ShaderProperty {
     ...
     * - color的长度为3 位置为2
     */
    linkBuffer(buffer: ShaderBuffer, size?: number, type?: BufferType, normalized?: boolean, stride?: number, position?: number) {
        this.buffer = buffer;
        this.size = size == undefined ? buffer.elementCount : size;
        this.bufferType = type == undefined ? BufferType.FLOAT : type;
     ...
     */
    bind() {
        this.buffer.pointer(this.location as number, this.size, this.bufferType, this.normalized, this.stride, this.position);
    }
}
