...
var VSHADER_SOURCE:string = null;
// Fragment shader program
var FSHADER_SOURCE:string = null;

class LoadShaderFromFiles {
  constructor() {
  ...
  // Read shader from file
  readShaderFile(gl, fileName, shader) {
    var request = new XMLHttpRequest();

    request.onreadystatechange =  ()=> {
      if (request.readyState === 4 && request.status !== 404) {
        this.onReadShader(gl, request.responseText, shader);
      }
    }
  ...
  onReadShader(gl, fileString, shader) {
    ...
    if (VSHADER_SOURCE && FSHADER_SOURCE) this.start(gl);
  }
}
