...
class RotatingTriangle_contextLost {
  constructor() {
    ...
    canvas.addEventListener('webglcontextrestored',  (ev)=> { this.start(canvas); }, false);
  ...
  // Current rotation angle
  private g_currentAngle = 0.0; // Changed from local variable to global variable
  private g_requestID; // The return value of requestAnimationFrame()
  ...
  start(canvas) {
    ...
    var modelMatrix = new cuonMatix.Matrix4();   // Create a model matrix

    var tick = () => {    // Start drawing
      this.g_currentAngle = this.animate(this.g_currentAngle);                // Update current rotation angle
      this.draw(gl, n, this.g_currentAngle, modelMatrix, u_ModelMatrix); // Draw the triangle
      this.g_requestID = requestAnimationFrame(tick);       // Reregister this Function again
    };
    tick();
  }

  contextLost(ev) { // Event Handler for context lost event
