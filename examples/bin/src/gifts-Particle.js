...
var FSHADER_SOURCE = '#ifdef GL_ES\n' +
    ...
    '}\n';
var g_perspectiveMatrix = new Utils.Matrix4();
var g_lightDir = new Utils.Vector3([0, 0.4, 0.6]);
var g_modelMatrix = new Utils.Matrix4();
var g_viewMatrix = new Utils.Matrix4();
...
var ParticleData = /** @class */ (function () {
    ...
    ParticleData.prototype.initParticle = function (wait) {
        // Movement speed
        var angle = Math.random() * Math.PI * 2;
        var height = Math.random() * 0.02 + 0.13;
        var speed = Math.random() * 0.01 + 0.02;
        this.velocity[0] = Math.cos(angle) * speed;
        this.velocity[1] = height;
        this.velocity[2] = Math.sin(angle) * speed;
        this.position[0] = Math.random() * 0.2;
        this.position[1] = Math.random() * 0.2;
        this.position[2] = Math.random() * 0.2;
        // Rotation angle
...
var Particle = /** @class */ (function () {
    function Particle() {
        ...
        // Set texture
        if (!this.initTextures(gl, "./resources/particle.png")) {
        ...
        this.init_gl(gl);
        this.sendQuadVertexBuffers(gl);
        // Create particles
        var particle = new Array(500);
        for (var i = 0; i < particle.length; ++i) {
            particle[i] = new ParticleData();
            particle[i].initParticle(true);
        }
        var tick = function () {
            window.requestAnimationFrame(tick);
            _this.updateParticle(particle);
            _this.drawCommon(gl, canvas, perspectiveMatrixShaderLocation, viewMatrixShaderLocation, lightDirShaderLocation);
            _this.drawParticle(gl, particle, perspectiveMatrixShaderLocation, modelMatrixShaderLocation, textureSamplerShaderLocation, alphaShaderLocation);
        };
        tick();
    }
    Particle.prototype.init_gl = function (gl) {
    ...
    Particle.prototype.drawCommon = function (gl, canvas, perspectiveMatrixShaderLocation, viewMatrixShaderLocation, lightDirShaderLocation) {
    ...
    Particle.prototype.drawParticle = function (gl, p, perspectiveMatrixShaderLocation, modelMatrixShaderLocation, textureSamplerShaderLocation, alphaShaderLocation) {
    ...
    Particle.prototype.updateParticle = function (p) {
        for (var i = 0; i < p.length; ++i) {
            // Wait for creation
            if (p[i].wait > 0) {
                p[i].wait--;
            ...
            p[i].alpha -= 0.05;
            if (p[i].alpha <= 0) {
                p[i].initParticle(false);
            }
        }
    };
    Particle.prototype.sendQuadVertexBuffers = function (gl) {
        //  v3----v2
        //  |      | 
        //  |      |
        //  |      |
        ...
        var g_quadVertexNormalBuffer = gl.createBuffer();
    ...
    Particle.prototype.initTextures = function (gl, str) {
        var _this = this;
        ...
        IMAGE.onload = function () { _this.loadTexture(gl, TEXTURE, IMAGE); };
        IMAGE.src = str;
        return true;
    };
    Particle.prototype.loadTexture = function (gl, texture, image) {
        gl.bindTexture(gl.TEXTURE_2D, texture);
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, image);
...
//# sourceMappingURL=gifts-Particle.js.map