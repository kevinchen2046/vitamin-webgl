...
declare interface WebGLRenderingContextBase {
    ...
    /**
     * 将缓冲区对象指定给attribute变量 
     * 步进和偏移函数
     * @param location 指定分配 attribute 中的存储地址
     * @param size 	指定缓冲区每个顶点分量的个数 1-4
     * @param type 	指定数据的类型
     * @param normalized 是否将非浮点型数据归一化 - 默认 true
     * @param stride 指定相邻两个顶点间的字节数 - 	默认 0
     * @param offset 指定缓冲区对象中的偏移量   -   默认 0
     */
    vertexAttribPointer(location: number, size: number, type: number, normalized: boolean, stride: number, offset: number);

    /**
     * 启用attribute变量
     * @param location 指定 attribute 的存储地址
     */
